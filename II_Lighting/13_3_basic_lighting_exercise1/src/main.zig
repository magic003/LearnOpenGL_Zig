...
var light_pos = zmath.loadArr3(.{ 1.2, 1.0, 2.0 });
...
pub fn main() !void {
    ...
    const lighting_shader = try learnopengl.Shader.init("shaders/13_3_basic_lighting.vs", "shaders/13_3_basic_lighting.fs");
    const light_cube_shader = try learnopengl.Shader.init("shaders/13_3_light_cube.vs", "shaders/13_3_light_cube.fs");
    ...
    while (!window.shouldClose()) {
        ...
        light_pos[0] = 1.0 + @sin(@as(f32, @floatCast(glfw.getTime()))) * 2.0;
        light_pos[1] = @sin(@as(f32, @floatCast(glfw.getTime())) / 2.0) * 1.0;
