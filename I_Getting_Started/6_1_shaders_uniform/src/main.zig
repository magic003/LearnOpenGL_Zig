...
pub fn main() !void {
    ...
    while (!window.shouldClose()) {
        ...
        const time_value = glfw.getTime();
        const green_value = std.math.sin(time_value) / 2.0 + 0.5;
        const vertext_color_location = gl.getUniformLocation(shader_program, "ourColor");
        gl.uniform4f(vertext_color_location, 0.0, @floatCast(green_value), 0.0, 1.0);
